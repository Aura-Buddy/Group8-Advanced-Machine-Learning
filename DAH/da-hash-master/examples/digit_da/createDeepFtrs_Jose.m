...
netRoot = 'C:/Users/ncson/OneDrive/Desktop/da-hash-master/da-hash-master/data';
dataRoot = 'C:/Users/ncson/OneDrive/Desktop/da-hash-master/da-hash-master/data/OfficeHome/OfficeHome';
...
% bittype = {'16';'64'};
bittype = {'64'};
clNames = {'Alarm_Clock', 'Backpack', 'Batteries', 'Bed', 'Bike', 'Bottle', 'Bucket', 'Calculator', 'Calendar', 'Candles'};
...
for domainpairindex = 1:length(ts)
   for bnumi = 1:length(bittype)
        ...
        srcDataDir = fullfile(dataRoot, srcDir);
        tgtDataDir = fullfile(dataRoot, tgtDir);
        metaPath = fullfile(dataRoot, 'meta.mat');
        saveFile = [srcDir,'-To-',tgtDir,'-HashDA',bnum,fc4,'.mat'];
        savePath = fullfile(netRoot, saveFile);
        labelNames_len = length(clNames);
        srcImages = {};
        srcLabels = {};
        tgtImages = {};
        ...
        for t = 1:bs:numel(imFileNames)
            ...
            test = net
            test1 = net.layers(end)
            test.layers(end) = test1
            test2 = imLabels(batch)
            test.layers(end).class = test2
            test.class = imLabels(batch)
            
            %net.layers{end}.class = imLabels(batch);
            
            dzdy = [];
            res = [];
            res = vl_simplenn_dah(test, im, dzdy, res, ...
                              'mode', evalMode, ...
            ...
            fprintf('\n');
            %ugh = squeeze(gather(res(11).x))
            fts{end+1} = squeeze(gather(res(11).x))
            if numGpus >= 1
