...
class TCA:
    ...
    def fit(self, Xs, Xt):
        ...
        A = V[:, ind[:self.dim]]
        Z = A.T @ K
        np.divide(Z, np.linalg.norm(Z, axis = 0), out = Z)
...
if __name__ == '__main__':
    ...
    domains = ['Caltech10_SURF_L10.mat', 'amazon_SURF_L10.mat',
               'webcam_SURF_L10.mat', 'dslr_SURF_L10.mat']
    for i in range(len(domains)):
        for j in range(len(domains)):
            if i != j:
                print("Source Domain: {}\nTarget Domain: {}".format(domains[i], domains[j]))
